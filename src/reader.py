...
def phan_loai_vung_roi(anchors: List[Dict],
    ...
    """Phân loại anchor thành các vùng ROI."""
...
def _cat_vung_roi(image: np.ndarray,
    ...
    if w <= 0 or h <= 0:
        raise ValueError(
            f"ROI không hợp lệ: ({x}, {y}, {w}, {h}), ảnh: {img_w}×{img_h}"
        )
...
def _read_code_grid_divide(binary: np.ndarray,
    ...
    """Fallback: chia đều ROI thành grid."""
